      ...
      if (ensureResult.list != null) {
        ...
        if (ensureResult.created) {
          ...
          if (isProject) { //Create some sample items
            ...
            list.items.inBatch(batch).add({ Title: "Training", Everyone: true, Category1: { results: ['Training']}}, entityTypeFullName).then(b => {
              console.log(b);
            });
            //, Category1: { results: ['Daily']}
            list.items.inBatch(batch).add({ Title: "Email triage", Everyone: true, Category1: { results: ['Daily']}}, entityTypeFullName).then(b => {
              console.log(b);
            });
            //, Category1: { results: ['Daily']}
            list.items.inBatch(batch).add({ Title: "Break", Everyone: true, Category1: { results: ['Daily']}}, entityTypeFullName).then(b => {
              console.log(b);
            });
            //, Category1: { results: ['Meetings']}
            list.items.inBatch(batch).add({ Title: "Team Meeting", Everyone: true, Category1: { results: ['Meetings']}}, entityTypeFullName).then(b => {
              console.log(b);
            });
            console.log('step6');         
            await batch.execute();
